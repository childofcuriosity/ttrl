...
class RayPPOTrainer:
    ...
    def _covalidate(self, temp=None): # 添加一个温度，非None就测测该温度的影响并且重命名
        ...
        Results are saved as CSV with prompt + groundtruth + final_vote + all votes.
        Parallelized following _validate(), not a per-sample loop.
        """
        import os
        import re
        ...
        # helper to count tokens robustly
        ...
        actor_model = self.actor_rollout_wg

        rollout_conf = self.config.actor_rollout_ref.rollout
        n_votes = self.config.ttrl.n_votes_per_prompt
 ...
 #debug_repeat_pel改4处：传参、惩罚因子、命名、调用处 。12.21该参数应该没用了，命名中去掉它。
        ...
        for test_data in self.val_dataloader:
            ...
            if "reward_model" in greedy_batch.non_tensor_batch:
                for rm in greedy_batch.non_tensor_batch["reward_model"]:
                    groundtruths.append(rm.get("ground_truth", ""))
            else:
                groundtruths = [""] * len(greedy_batch)
            ...
            with torch.no_grad():
                greedy_output_padded = actor_model.generate_sequences(greedy_batch_padded)
                greedy_output = unpad_dataproto(greedy_output_padded, pad_size=pad_size)

            greedy_outputs = [self.tokenizer.decode(ids, skip_special_tokens=True)
                            for ids in greedy_output.batch["responses"]]
            print("[COVALIDATE] greedy generation end")
            ...
            for i, gens in enumerate(grouped):
                print(i)
                ...
                # greedy fields
                row['greedy'] = greedy_resp
                row['greedy_len'] = token_count(greedy_resp)
                greedy_extracted = extract_answer(greedy_resp) if greedy_resp is not None else ""
                row['greedy_extract'] = greedy_extracted if greedy_extracted is not None else ""
                row['greedy_extract_len'] = token_count(row['greedy_extract'])
                # greedy_correct: try grade, fallback to string equality
                row['greedy_correct'] = grade(row['greedy_extract'], gt, fast=False)

                # pseudo (representative of votes)
                row['pseudo'] = final_vote
                row['pseudo_len'] = token_count(final_vote)
                row['pseudo_correct'] = grade(row['pseudo'], gt, fast=False)

                # equivalence_groups_list_num: list of (group_list, count) sorted by count desc
                # build mapping root->group_list
                root_to_group = {uf.find(x): group for group in equivalence_groups for x in group}
                groups_with_counts = []
                for root, cnt in vote_counter.most_common():
                ...
                votes_extract_len = [token_count(ea) for ea in votes_extract]
                votes_correct = []
                for ve in votes_extract:
                    votes_correct.append(grade(ve, gt, fast=False))

                row['votes_len'] = votes_len
                row['votes_extract'] = votes_extract
                row['votes_extract_len'] = votes_extract_len
                row['votes_correct'] = votes_correct
                row['votes'] = votes
        ...
        # pandas will keep the OrderedDict column order when creating DataFrame
        pd.DataFrame(results).to_csv(csv_path, index=False)
        print(f"[COVALIDATE] Saved {len(results)} samples to {csv_path}")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # # Debug-guard: if union raises AssertionError due to mismatched batch sizes,
                    # # save offending objects and drop into pdb for interactive inspection.
                    # try:
                    #     batch = batch.union(gen_batch_output)
                    # except AssertionError as e:
                    #     import pdb, traceback, time, os
                    #     print("AssertionError during DataProto.union():", e)
                    #     try:
                    #         bs1 = getattr(batch.batch, "batch_size", None)
                    #     except Exception:
                    #         bs1 = None
                    #     try:
                    #         bs2 = getattr(gen_batch_output.batch, "batch_size", None)
                    #     except Exception:
                    #         bs2 = None
                    #     print(f"batch.batch.batch_size = {bs1}, gen_batch_output.batch.batch_size = {bs2}")

                    #     # persist the problematic objects for offline inspection
                    #     dbg_dir = os.path.join(getattr(self.config.trainer, "default_local_dir", "."), "debug_batches")
                    #     os.makedirs(dbg_dir, exist_ok=True)
                    #     ts = int(time.time())
                    #     try:
                    #         torch.save(batch, os.path.join(dbg_dir, f"batch_{self.global_steps}_{ts}.pt"))
                    #         torch.save(gen_batch_output, os.path.join(dbg_dir, f"gen_batch_output_{self.global_steps}_{ts}.pt"))
                    #         print(f"Saved debug batches to {dbg_dir}")
                    #     except Exception as save_e:
                    #         print("Failed to save debug batches:", save_e)

                    #     traceback.print_exc()
                    #     # NOTE: pdb in Ray actors is interactive only when running in local_mode or attached terminal.
                    #     # If you want an interactive pdb, run ray.init(local_mode=True) or inspect saved .pt files.
                    #     pdb.set_trace()
                    #     raise
